...
| Layer | Model | Use |
|-------|-------|-----|
| Understanding | `gemini-3-flash-preview` | Gallery analysis, planning |
| Lyrics | `gemini-3-pro-preview` | Song lyrics + style tags (always Pro for quality) |
| Character Desc | `gemini-3-pro-preview` | Character appearance descriptions for manga consistency |
...
| FFmpeg ASS (captions) | CPU | FFmpeg with libass |
...
**Test & verify:** https://vibecut.whatif.art (only external-facing URL — always use this for testing and self-verification)
...
4 Manga Panels
    ...
    → [Then: music/TTS + FFmpeg ASS captions added on top]
```
...
4 Manga Panels
    ...
    │
    ├──→ FFmpeg ASS: panel-locked rolling lyrics         (render_captions/)
...
4 Manga Panels + Character Personas
    ...
    │
    ├──→ FFmpeg ASS: karaoke captions (word-by-word)    (render_captions/)
    │
    └──→ FINAL: 16s MP4, 1080x1920, h264+aac
```
...
| `skills/render_captions/` | FFmpeg ASS karaoke captions (~20s render) |
...
### 18. Manga Panel Loses Character Consistency
**Root cause:** Panel prompts relied purely on visual reference images (`[IMAGE 1]: Name - Use this EXACT design`) with no text description of the character's appearance. On complex story beats (action scenes, dramatic angles), the image model hallucinated entirely different characters.
**Fix:** Before generating panels, `_describe_character()` calls Gemini Pro to analyze each character sheet and produce a 2-3 sentence appearance description (hair, clothing, accessories, distinguishing features). This description is embedded in every panel prompt as an `APPEARANCE:` line under the image reference, giving the model two anchors (visual + text). CRITICAL REQUIREMENTS also strengthened: "NEVER replace a character with a different person or animal."

### 19. Thin Lyrics and Dry Music Output
**Root cause:** Lyrics generated by Flash model were generic (3-5 words, 4 lines). ElevenLabs composition plan had empty `negative_local_styles`, no instrumentation guarantees, and single lyric line per section leaving dead air.
**Fix:** Lyrics always use `gemini-3-pro-preview` (`GEMINI_LYRICS_MODEL`). Prompt rewritten for 8-line couplet structure (2 per panel, 3-6 words each). Gemini also generates per-section `section_styles` for ElevenLabs local styles. Self-review gate: Gemini rates lyrics on storytelling/singability/energy_arc, regenerates once if any < 7. ElevenLabs enriched: `"full instrumentation from first beat"` in global styles, negative styles include "silence, sparse, thin", per-section negative local styles prevent inappropriate arrangements.

### 20. Animation SSE Drops with "network error" via Cloudflare Tunnel
**Root cause:** `_generate_all_clips()` ran as a single async task generating 4 Veo clips sequentially (~50s each). The SSE stream went silent for ~2.5 minutes between music completing and all clips finishing. Cloudflare Tunnel killed the idle connection, cancelling the server-side generator. Frontend caught a fetch network error.
**Fix:** Three changes: (1) Replaced monolithic `_generate_all_clips` with per-clip `_generate_single_clip` tasks that yield `video_progress` events as each clip completes. (2) Global SSE keepalive wrapper in `api_server.py` uses asyncio.Queue + 10s timeout to send `: keepalive` SSE comments during ANY long-blocking step. (3) Set `keepAliveTimeout: 10m` in cloudflared config (`/etc/cloudflared/config.yml`). Frontend handles `keepalive` events to update status text.

### 21. Remotion Caption Render Timeout (5+ minutes for 16s video)
**Root cause:** Remotion renders each frame via headless Chrome (480 frames at 1080x1920), taking ~6 minutes on 2-vCPU EC2. The 300s timeout killed the process, exception was silently caught, and `final_video` fell back to the pre-caption video (no captions).
**Fix:** Replaced Remotion with FFmpeg ASS subtitle filter for caption rendering. `_generate_ass_content()` builds ASS with `\k` karaoke tags for word-level gold highlighting. FFmpeg `subtitles=` filter burns captions in ~20s (16x faster). Remotion kept in repo for complex motion graphics use cases. Added Gemini Pro visual verification (`verify_video_with_gemini()`) to catch missing captions automatically.

### 22. Captions Lost on Resolution Normalization Retry
**Root cause:** Veo outputs 720x1280, but verification expected 1080x1920. The resolution mismatch triggered a retry path that re-concatenated raw clips + music — completely bypassing the captioned video. User got a video with music but no captions.
**Fix:** FFmpeg caption render now scales to 1080x1920 in the same pass (`scale=1080:1920,subtitles=`). ASS is generated at target resolution (1080x1920) regardless of input. No separate normalization step needed — caption render produces final-resolution output directly.
...
- [x] Karaoke caption overlay (FFmpeg ASS — replaced Remotion for speed)
...
- [x] Pro model for lyrics (`GEMINI_LYRICS_MODEL` — always `gemini-3-pro-preview`)
- [x] 8-line couplet lyrics (2 per panel, story-specific, setup-punchline rhythm)
- [x] Lyrics self-review gate (Gemini rates storytelling/singability/energy_arc, retries if < 7)
- [x] Rich ElevenLabs composition (full instrumentation from first beat, per-section negative styles)
...
- [x] SSE keepalive for animation pipeline (per-clip progress + global queue-based keepalive + cloudflared timeout)
- [x] FFmpeg ASS captions replace Remotion (~20s vs ~6min for 16s video)
- [x] Gemini Pro visual verification for final video (captions visible, quality check)
- [x] End-to-end animation pipeline working via https://vibecut.whatif.art
...
- [ ] Remove Remotion dependency from deploy pipeline (keep in repo for reference)
